...
public class Camera implements IUpdateHandler {
	...
	public void setSurfaceSize(final int pSurfaceX, final int pSurfaceY,
		...
		if(this.mSurfaceHeight == 0 && this.mSurfaceWidth == 0) {
			...
			this.onSurfaceSizeChanged(this.mSurfaceX, this.mSurfaceY, this.mSurfaceWidth,
					this.mSurfaceHeight, pSurfaceX, pSurfaceY, pSurfaceWidth, pSurfaceHeight);
		}
	}
	...
	public boolean isRectangularShapeVisible(final float pX, final float pY, final float pWidth,
											 final float pHeight, final Transformation
													 pLocalToSceneTransformation) {
		return RectangularShapeCollisionChecker.isVisible(this, pX, pY, pWidth, pHeight,
				pLocalToSceneTransformation);
	}

	public void onApplySceneMatrix(final GLState pGLState) {
		pGLState.orthoProjectionGLMatrixf(this.getXMin(), this.getXMax(), this.getYMax(),
				this.getYMin(), this.mZNear, this.mZFar);
	...
	public void onApplyCameraSceneMatrix(final GLState pGLState) {
		...
		if(cameraSceneRotation != 0) {
			Camera.applyRotation(pGLState, widthRaw * 0.5f,
					heightRaw * 0.5f, cameraSceneRotation);
		}
	}

	private static void applyRotation(final GLState pGLState, final float pRotationCenterX,
									  final float pRotationCenterY, final float pAngle) {
	...
	public float[] getSceneCoordinatesFromCameraSceneCoordinates(final float pCameraSceneX,
																 final float pCameraSceneY) {
	...
	private void applySceneRotation(final TouchEvent pCameraSceneTouchEvent) {
		...
		if(rotation != 0) {
			...
			MathUtils.rotateAroundCenter(Camera.VERTICES_TMP, -rotation, this.getCenterX(),
					this.getCenterY());

			pCameraSceneTouchEvent.set(Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X],
	...
	private void unapplySceneRotation(final float[] pSceneCoordinates) {
		...
		if(rotation != 0) {
			MathUtils.revertRotateAroundCenter(pSceneCoordinates, rotation, this.getCenterX(),
					this.getCenterY());
		}
	}
