...
public class ZoomCamera extends BoundCamera {
	...
	protected void applySceneToCameraSceneOffset(final TouchEvent pSceneTouchEvent) {
		...
		if(zoomFactor != 1) {
			...
			MathUtils.scaleAroundCenter(Camera.VERTICES_TMP, zoomFactor, zoomFactor,
					this.getCenterX(), this.getCenterY()); // TODO Use a Transformation object instead!?!
	...
	protected void unapplySceneToCameraSceneOffset(final TouchEvent pCameraSceneTouchEvent) {
		...
		if(zoomFactor != 1) {
			...
			MathUtils.revertScaleAroundCenter(Camera.VERTICES_TMP, zoomFactor, zoomFactor,
	...
	protected void unapplySceneToCameraSceneOffset(final float[] pCameraSceneCoordinates) {
		...
		final float zoomFactor = this.mZoomFactor;
		if(zoomFactor != 1) {
			MathUtils.revertScaleAroundCenter(pCameraSceneCoordinates, zoomFactor, zoomFactor,
					this.getCenterX(), this.getCenterY());
		}
	}
